...
async function getGamesFromDb() {
    ...
    const uid = user.uid;
    return await firebase.database().ref().child(`games/${uid}`).once('value').then(
        function (snapshot) {
            const games = snapshot.val()
            if (games) {
                for (const gameName of Object.keys(games)) {
                    games[gameName].defs = initEmptyRanges(games[gameName].defs)
                    games[gameName].defs = offsetStrsToList(games[gameName].defs);
