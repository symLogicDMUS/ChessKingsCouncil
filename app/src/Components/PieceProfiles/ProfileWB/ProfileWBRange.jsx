...
function ProfileWBRange(props) {
    const { pieceName, pieceImgUrl, theme, color, range, rangeType, adjust } =
        props;

    const rangeBoardImgStr = useMemo(
        () => getRangeBoardImgStr("d4", rangeType, range, theme),
        [theme, range.length]
    );
    ...
    return (
        ...
        >
            <Box className={clsx(classes2.sqr_item_header, {})}>
                <Typography
                    variant="caption"
                    className={classes2.sqr_item_title}
                >
            ...
            >
                ...
                />
                <div
                    className={clsx(classes.board_grid, {
                        [classes.adjust]: adjust,
                    })}
                >
                    <img
                        src={pieceImgUrl}
                        className={classes.piece_img}
                        alt={`image of piece ${pieceName}`}
                    />
                </div>
            </Box>
        </Box>
