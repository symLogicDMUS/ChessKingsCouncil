...
def sqr_under_attack(board, sqr, color, piece_defs, id_dict):
    """return True if square is under attack, else False """
    threat_area = get_threat_area(board, sqr, color, piece_defs, id_dict)
...
if __name__ == "__main__":
    ...
    for y in range(1, 9):
        for x in range(1, 9):
            results[(x, y)] = sqr_under_attack(board, (x, y), 'W', piece_defs, id_dict)
    ...
    for y in range(1, 9):
        for x in range(1, 9):
            results[(x, y)] = sqr_under_attack(board, (x, y), 'B', piece_defs, id_dict)
    under_attack = list(filter(lambda sqr: results[sqr], results.keys()))
    not_under_attack = list(filterfalse(lambda sqr: results[sqr], results.keys()))
    ...
    for sqr in [(5, 5), (6, 5), (7, 5), (5, 6), (7, 6), (5, 7), (6, 7), (7, 7)]:
        if sqr_under_attack(board, sqr, 'W', piece_defs, id_dict):
            threats.append(sqr)
    ...
    for hypo_k_loc, hypo_board in hypo_boards.items():
        if sqr_under_attack(hypo_board, hypo_k_loc, 'W', piece_defs, id_dict):
