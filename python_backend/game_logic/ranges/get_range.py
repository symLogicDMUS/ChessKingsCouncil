...
def get_range(board, sqr, color, piece_defs, id_dict):
    """ """
    id_ = board[sqr]
    fen_id = id_[1].lower()
    name = id_dict[fen_id]
    def_ = piece_defs[name]
...
if __name__ == "__main__":
    ...
    piece_range = get_range(id_defs, piece_defs, board, (3, 3), 'W')
    print_board(board, heading="c3", highlights=piece_range)
    piece_range = get_range(id_defs, piece_defs, board, (4, 5), 'W')
