...
def get_king_actual_moves(board, king_loc, init_range, color, piece_defs, id_dict):
    ...
    for hypo_k_loc, hypo_board in boards.items():
        if not sqr_under_attack(hypo_board, hypo_k_loc, color, piece_defs, id_dict):
